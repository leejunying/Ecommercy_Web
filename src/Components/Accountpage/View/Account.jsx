...
const Account = () => {
  ...
  const Sendreset = () => {
    axios
      .post("http://localhost:4000/account/reset", {
        Email: resetmail,
      })
      .then((res) => {
        return res.data;
  ...
  const Register = (value) => {
    ...
    axios
      .post("http://localhost:4000/account/signup", value)
      .then((res) => {
        return res.data;
      })
      .then((mess) => {
  ...
  const Profile = (token) => {
    axios
      .post(
        "http://localhost:4000/account/profile",
        {},
        {
          headers: {
  ...
  const Login = (value) => {
    axios
      .post("http://localhost:4000/account/login", value)
      .then((res) => {
        return res.data;
      })
      .then((mess) => {
      ...
      <Grid container={true} className="Login">
        ...
        <Grid container={true} className="flex sp-between Account-option">
          <Grid
            items={true}
            md={6}
            classname="flex col jus-start Register-form"
          >
            <label>
              If you don't have an account please Register in form bellow
            </label>
            ...
            <from name="Register">
              <Grid container={true} md={5} className="flex col">
                <TextField
                  ...
                  helperText={regfirstname.notice}
                  id="standard-basic"
                  label="First name"
                  autoFocus
                  onChange={(e) => {
                    handleInputRes("first", e);
                  }}
                />
                <TextField
                  value={reglastname.value}
                  error={reglastname.error}
                  helperText={reglastname.notice}
                  id="standard-basic"
                  label="Last name"
                  autoFocus
                  onChange={(e) => {
                    handleInputRes("last", e);
                  }}
                />
                <TextField
                  value={regemail.value}
                  error={regemail.error}
                  helperText={regemail.notice}
                  id="standard-basic"
                  required
                  label="Email"
                  autoFocus
                  onChange={(e) => {
                    handleInputRes("email", e);
                  }}
                />
                <TextField
                  value={regpassword.value}
                  error={regpassword.error}
                  helperText={regpassword.notice}
                  type="password"
          ...
          <Grid
            container={true}
            md={6}
            classname="flex col jus-start Login-form "
          >
            <Grid>
              ...
              <form>
                <Grid className=" flex col">
                  <TextField
                    ...
                    helperText={logemail.notice}
                    required
                    label="Email"
                    onChange={(event) => {
                      handleInputLog("email", event);
                    }}
                  />
                  <TextField
                    value={logpassword.value}
                    error={logpassword.error}
                    helperText={logpassword.notice}
                    type="password"
                    id="standard-basic"
                    autoFocus
                    autoComplete="current-password"
