...
io.on("connection", async (socket) => {
  ...
  var roomid=count+1
    ...
    socket.on('Client',async (data)=>{
      ...
      if(data=='Client')
      {
        ...
        let result= await Controller_chat.Createchatroom(roomid,[guestid,'admin'])
    ...
    socket.on('admin_message',async(message)=>{
      ...
      let listroom= await Controller_chat.Getroom()

      let foundroom=listroom.filter(foundroom=>foundroom.Room==message.Room)

      io.emit(`res_${foundroom.Room}`,addnewmessage)
  ...
  socket.on("disconnect", async function () {
       ...
       await Controller_chat.Removeroom(roomid)
       await Controller_chat.Removemessage(roomid)
      console.log(`${roomid!=0?roomid:'admin disconnected'}`)
      io.emit('Client_off',`${roomid!=0?roomid:' Offline'}`)
